...
impl Rune {
	...
	pub fn reserved(block: u64, tx: u32) -> Self {
		Self(Self::RESERVED.checked_add(u128::from(block) << 32 | u128::from(tx)).unwrap())
	}
...
impl Display for Rune {
	fn fmt(&self, f: &mut Formatter) -> fmt::Result {
		...
		while n > 0 {
			symbol.push("ABCDEFGHIJKLMNOPQRSTUVWXYZ".chars().nth(((n - 1) % 26) as usize).unwrap());
			n = (n - 1) / 26;
		}
...
mod tests {
	...
	#[test]
	fn from_str_error() {
		assert_eq!("BCGDENLQRQWDSLRUGSNLBTMFIJAW".parse::<Rune>().unwrap_err(), Error::Range,);
		assert_eq!("BCGDENLQRQWDSLRUGSNLBTMFIJAVX".parse::<Rune>().unwrap_err(), Error::Range,);
	...
	fn minimum_at_height() {
		...
		fn case(network: Network, height: u32, minimum: &str) {
			assert_eq!(Rune::minimum_at_height(network, Height(height)).to_string(), minimum,);
		}

		case(Network::Testnet, 0, "AAAAAAAAAAAAA");
		case(Network::Testnet, SUBSIDY_HALVING_INTERVAL * 12 - 1, "AAAAAAAAAAAAA");
		case(Network::Testnet, SUBSIDY_HALVING_INTERVAL * 12, "ZZYZXBRKWXVA");
		case(Network::Testnet, SUBSIDY_HALVING_INTERVAL * 12 + 1, "ZZXZUDIVTVQA");
	...
	#[test]
	fn reserved() {
		assert_eq!(Rune::RESERVED, "AAAAAAAAAAAAAAAAAAAAAAAAAAA".parse::<Rune>().unwrap().0,);
