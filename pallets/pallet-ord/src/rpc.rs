...
use std::str::FromStr;
use bitcoin::{Block, BlockHash, consensus};
		...
		R: for<'a> Deserialize<'a> + std::fmt::Debug,
{
	...
	let mut request = http::Request::post(url, vec![data.clone()])
		.add_header("Content-Type", "application/json");
	let  pending = request
		.deadline(deadline)
		.body(vec![data])
		.send().map_err(|_| OrdError::OffchainHttp(http::Error::IoError))?;
...
pub(crate) fn get_block(url: &'static str, hash: BlockHash) -> Result<Block> {
	let payload = request_payload("getblock",
...
pub(crate) fn get_raw_tx(url: &'static str, txid: Txid) -> Result<GetRawTransactionResult> {
	...
	let payload = request_payload("getrawtransaction",
								  serde_json::json!([format!("{:x}", value), true]));
