...
impl Sat {
	...
	pub fn name(self) -> String {
		...
		while x > 0 {
			name.push("abcdefghijklmnopqrstuvwxyz".chars().nth(((x - 1) % 26) as usize).unwrap());
			x = (x - 1) / 26;
		}
	...
	fn from_degree(degree: &str) -> Result<Self, Error> {
		let (cycle_number, rest) =
			degree.split_once('°').ok_or_else(|| ErrorKind::MissingDegree.error(degree))?;

		let cycle_number = cycle_number
			.parse::<u32>()
			.map_err(|source| ErrorKind::ParseInt { source }.error(degree))?;

		let (epoch_offset, rest) =
			rest.split_once('′').ok_or_else(|| ErrorKind::MissingMinute.error(degree))?;
		...
		let (period_offset, rest) =
			rest.split_once('″').ok_or_else(|| ErrorKind::MissingSecond.error(degree))?;
	...
	fn from_decimal(decimal: &str) -> Result<Self, Error> {
		let (height, offset) =
			decimal.split_once('.').ok_or_else(|| ErrorKind::MissingPeriod.error(decimal))?;

		let height =
			Height(height.parse().map_err(|source| ErrorKind::ParseInt { source }.error(decimal))?);
...
impl ErrorKind {
	fn error(self, input: &str) -> Error {
		Error { input: input.to_string(), kind: self }
	}
}
...
impl FromStr for Sat {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		if s.chars().any(|c| c.is_ascii_lowercase()) {
			...
			let sat = Self(s.parse().map_err(|source| ErrorKind::ParseInt { source }.error(s))?);
			if sat > Self::LAST {
...
mod tests {
	...
	fn height() {
		...
		assert_eq!(Epoch(2).starting_sat().height(), SUBSIDY_HALVING_INTERVAL * 2);
	...
	fn degree() {
		...
		assert_eq!(Sat(50 * COIN_VALUE - 1).degree().to_string(), "0°0′0″4999999999‴");
		assert_eq!(Sat(50 * COIN_VALUE).degree().to_string(), "0°1′1″0‴");
		assert_eq!(Sat(50 * COIN_VALUE + 1).degree().to_string(), "0°1′1″1‴");
		assert_eq!(
			Sat(50 * COIN_VALUE * u64::from(DIFFCHANGE_INTERVAL) - 1).degree().to_string(),
			"0°2015′2015″4999999999‴"
		);
		assert_eq!(
			Sat(50 * COIN_VALUE * u64::from(DIFFCHANGE_INTERVAL)).degree().to_string(),
			"0°2016′0″0‴"
		);
		assert_eq!(
			Sat(50 * COIN_VALUE * u64::from(DIFFCHANGE_INTERVAL) + 1).degree().to_string(),
		...
		assert_eq!(
			Sat(50 * COIN_VALUE * u64::from(SUBSIDY_HALVING_INTERVAL)).degree().to_string(),
			"0°0′336″0‴"
		);
		assert_eq!(
			Sat(50 * COIN_VALUE * u64::from(SUBSIDY_HALVING_INTERVAL) + 1)
		...
		assert_eq!(Sat(2067187500000000 - 1).degree().to_string(), "0°209999′2015″156249999‴");
	...
	fn invalid_degree_bugfix() {
		...
		assert_eq!(Sat(1914226250000000).degree().to_string(), "0°122762′794″0‴");
	...
	fn epoch() {
		...
		assert_eq!(Sat(50 * COIN_VALUE * u64::from(SUBSIDY_HALVING_INTERVAL)).epoch(), 1);
	...
	fn subsidy_position() {
		...
		assert_eq!(Sat(Height(0).subsidy() - 1).third(), Height(0).subsidy() - 1);
		assert_eq!(Sat(Height(0).subsidy()).third(), 0);
		assert_eq!(Sat(Height(0).subsidy() + 1).third(), 1);
		assert_eq!(Sat(Epoch(1).starting_sat().n() + Epoch(1).subsidy()).third(), 0);
	...
	#[test]
	fn cycle() {
		assert_eq!(SUBSIDY_HALVING_INTERVAL * CYCLE_EPOCHS % DIFFCHANGE_INTERVAL, 0);

		for i in 1..CYCLE_EPOCHS {
			assert_ne!(i * SUBSIDY_HALVING_INTERVAL % DIFFCHANGE_INTERVAL, 0);
		}

		assert_eq!(CYCLE_EPOCHS * SUBSIDY_HALVING_INTERVAL % DIFFCHANGE_INTERVAL, 0);
	...
	fn error_display() {
		assert_eq!(
			Error { input: "foo".into(), kind: ErrorKind::Percentile }.to_string(),
