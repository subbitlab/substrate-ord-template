	...
	pub fn decipher(transaction: &Transaction) -> Option<Artifact> {
		let payload = match Runestone::payload(transaction) {
			...
			Some(Payload::Invalid(flaw)) => {
				return Some(Artifact::Cenotaph(Cenotaph { flaw: Some(flaw), ..default() }));
			},
			None => return None,
		};

		let Ok(integers) = Runestone::integers(&payload) else {
			return Some(Artifact::Cenotaph(Cenotaph { flaw: Some(Flaw::Varint), ..default() }));
		};

		let Message { mut flaw, edicts, mut fields } =
			Message::from_integers(transaction, &integers);

		let mut flags = Tag::Flags.take(&mut fields, |[flags]| Some(flags)).unwrap_or_default();
		...
		let etching = Flag::Etching.take(&mut flags).then(|| Etching {
			...
			symbol: Tag::Symbol
				.take(&mut fields, |[symbol]| char::from_u32(u32::try_from(symbol).ok()?)),
			terms: Flag::Terms.take(&mut flags).then(|| Terms {
				cap: Tag::Cap.take(&mut fields, |[cap]| Some(cap)),
				height: (
					Tag::HeightStart
						.take(&mut fields, |[start_height]| u64::try_from(start_height).ok()),
					Tag::HeightEnd
						.take(&mut fields, |[start_height]| u64::try_from(start_height).ok()),
				),
				...
				offset: (
					Tag::OffsetStart
						.take(&mut fields, |[start_offset]| u64::try_from(start_offset).ok()),
		...
		if etching.map(|etching| etching.supply().is_none()).unwrap_or_default() {
		...
		Some(Artifact::Runestone(Self { edicts, etching, mint, pointer }))
	}
...
mod tests {
	...
	fn invalid_input_scripts_are_skipped_when_searching_for_runestone() {
		...
		assert_eq!(
			Runestone::decipher(&Transaction {
				input: Vec::new(),
				output: vec![
					TxOut { script_pubkey: ScriptBuf::from_bytes(script_pubkey), value: 0 },
					TxOut {
						script_pubkey: script::Builder::new()
							.push_opcode(opcodes::all::OP_RETURN)
							.push_opcode(Runestone::MAGIC_NUMBER)
			...
			.unwrap(),
			Artifact::Runestone(Runestone { mint: Some(RuneId::new(1, 1).unwrap()), ..default() }),
		);
	}
	...
	fn runestone_may_be_in_second_output() {
		...
		assert_eq!(
			Runestone::decipher(&Transaction {
				input: Vec::new(),
				output: vec![
					TxOut { script_pubkey: ScriptBuf::new(), value: 0 },
					TxOut {
						script_pubkey: script::Builder::new()
							.push_opcode(opcodes::all::OP_RETURN)
							.push_opcode(Runestone::MAGIC_NUMBER)
	...
	fn runestone_size() {
		...
		fn case(edicts: Vec<Edict>, etching: Option<Etching>, size: usize) {
			assert_eq!(Runestone { edicts, etching, ..default() }.encipher().len(), size);
		}
	...
	fn encipher() {
		...
		fn case(runestone: Runestone, expected: &[u128]) {
			...
			let transaction = Transaction {
				input: Vec::new(),
				output: vec![TxOut { script_pubkey, value: 0 }],
				lock_time: LockTime::ZERO,
				version: 2,
			};
			...
			let runestone = {
				...
				edicts.sort_by_key(|edict| edict.id);
				Runestone { edicts, ..runestone }
			};

			assert_eq!(Runestone::decipher(&transaction).unwrap(), Artifact::Runestone(runestone),);
		}
	...
	fn invalid_spacers_does_not_produce_cenotaph() {
		assert_eq!(decipher(&[Tag::Spacers.into(), u128::MAX]), Artifact::Runestone(default()),);
	}

	#[test]
	fn invalid_symbol_does_not_produce_cenotaph() {
		assert_eq!(decipher(&[Tag::Symbol.into(), u128::MAX]), Artifact::Runestone(default()),);
	}
		...
		assert_eq!(
			Runestone::decipher(&Transaction {
				...
				output: vec![
					...
					TxOut { script_pubkey: Runestone::default().encipher(), value: 0 }
				],
			})
			.unwrap(),
			Artifact::Runestone(Runestone::default()),
		...
		for i in 0..79 {
			...
			assert_eq!(
				Runestone::decipher(&Transaction {
					...
					lock_time: LockTime::ZERO,
					input: default(),
					output: vec![TxOut { script_pubkey: script_pubkey.into(), value: 0 },],
				})
				.unwrap(),
				Artifact::Runestone(Runestone::default()),
			);
